...
man_lines = [
    ...
    '.TP',
    '.B \\-\\-no\\-plot',
    'Suppress the 25-row battery depletion chart and its lid-event markers at exit. Battery discovery, fifteen-second sampling, synchronized log records, per-battery statistics, and detected lid-transition counts remain enabled.',
    '.TP',
    '.BR \\-h ", " \\-\\-help',
    ...
    '.PP',
    'The X axis uses elapsed-time ordering. With --ignore-lid, it includes both sample-attempt and detected lid-event times so every event fits; otherwise sample attempts alone establish the range. Both boundaries always label the actual local wall-clock HH:mm of the earliest and latest entries, including missing readings or events outside the sample interval. Both endpoint callouts are reserved before interior labels, even when their displayed times are equal. If the entire plotted timeline has one elapsed value, its true endpoint labels repeat without inventing a duration and its points remain at the left. Boundary ticks align with the axis edges; interior ticks mark actual elapsed-time positions, and adjacent callouts have at least two blank columns. Missing readings remain gaps.',
    '.PP',
