...
man_lines = [
    ...
    '.TP',
    '.B \\-\\-no\\-plot',
    'Suppress the 25-row battery depletion chart at exit. Battery discovery, fifteen-second sampling, synchronized log records, and per-battery statistics remain enabled.',
    '.TP',
    '.BR \\-h ", " \\-\\-help',
    ...
    'Burnbag discovers present entries of type Battery under /sys/class/power_supply in lexical kernel-name order and monitors up to two independently. It takes an initial reading before operational host mutation, samples persistent run modes every 15 seconds, and takes a final reading as handled teardown begins. One-shot modes receive initial and final readings. Percentage ordering and rate calculations use suspend-inclusive Linux CLOCK_BOOTTIME; local wall clock is presentation-only. Systems without a battery continue normally and omit battery output.',
    '.PP',
    ...
    '.PP',
    'The handled-exit chart contains exactly 25 data rows and uses the current standard-output terminal width, with an 80-column fallback. Its Y axis spans only observed percentage values and labels at most one observed value per row. Its X axis uses monotonic ordering, marks real sample positions with ticks, and labels selected actual local times as HH:mm. A real endpoint is retained when it fits, and adjacent callouts have at least two blank columns. Missing readings remain gaps.',
    '.PP',
