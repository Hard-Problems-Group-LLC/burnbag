...
man_lines = [
    ...
    '.PP',
    'The X axis uses elapsed-time ordering. Both boundaries always label the actual local wall-clock HH:mm of the earliest and latest sample attempts, including missing readings at either edge. Both endpoint callouts are reserved before interior labels, even when their displayed times are equal. A lone sample or a history with equal elapsed times repeats its true endpoint labels without inventing a duration, while its points remain at the left. Boundary ticks align with the axis edges; interior ticks mark actual elapsed-time sample positions, and adjacent callouts have at least two blank columns. Missing readings remain gaps.',
    '.PP',
